                ...
                if (markers) { // neu markers tạo mới tồn tại thực hiện cập nhật vị trí markers khi user nhập địa chỉ
                    ...
                    map.on('popupopen', async function (e) {
                        ...
                        if (marker == markers) {
                            ...
                            // console.log(bfb)
                            document.getElementById(`buttonfixbin${index}`).addEventListener('click', checkUpdate)
                            // if(closePopup) {
                            if (ab) {
                                //xóa đi event remove thừa
                                ab._events.remove.splice(0, 1, ab._events.remove[1]);
                                console.log(ab._events.remove)

                            }
                            // bắt sự kiện popup đóng 
                            ab = markers.getPopup().on('remove', function () {

                                console.log('kkkk')
                                // xóa sự kiện sửa ttin để khi bật popup lên add sự kiện không bị lặp
                                document.getElementById(`buttonfixbin${index}`).removeEventListener('click', checkUpdate)
                            });


                            document.getElementById(`buttoncancelfixbin${index}`).addEventListener('click', async () => {
                                map.removeLayer(layerGroup)
                                await props.getBin();
